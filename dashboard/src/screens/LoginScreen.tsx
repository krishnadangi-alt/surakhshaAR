...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen w-screen relative flex flex-col justify-between items-center p-6 font-sans overflow-hidden bg-slate-950">
      {/* Jharkhand Mining Site Background Image */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat transition-all duration-700 scale-105"
        style={{ backgroundImage: `url('/jharkhand_mining_bg.jpg')` }}
      />

      {/* Dark Vignette & Atmospheric Overlay for high readability */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#09111E]/85 via-[#0B1A30]/75 to-[#09111E]/90 backdrop-blur-[2px]" />

      {/* Top Institutional Header */}
      <header className="w-full max-w-md pt-6 text-center z-10">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0F172A]/85 border border-slate-700/80 text-xs font-semibold text-slate-200 shadow-xl backdrop-blur-md">
      ...
      <div className="w-full max-w-md my-auto z-10">
        <div className="rounded-2xl border border-slate-700/80 bg-[#0F172A]/90 p-8 shadow-2xl backdrop-blur-xl relative">
          {/* Top Amber Accent Line */}
          <div className="absolute top-0 left-8 right-8 h-[2px] bg-gradient-to-r from-transparent via-[#F59E0B] to-transparent opacity-90" />
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••••••"
                  className="w-full rounded-xl border border-slate-700 bg-slate-800/80 pl-10 pr-4 py-2.5 text-xs font-semibold text-white focus:border-[#1D6BF3] focus:outline-none focus:ring-1 focus:ring-[#1D6BF3]"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#1D6BF3] py-3 text-xs font-bold text-white hover:bg-[#1556C7] transition shadow-lg mt-6 disabled:opacity-50"
            >
              {isLoading ? (
